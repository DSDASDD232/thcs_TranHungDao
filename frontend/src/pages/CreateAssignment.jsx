...
// 👉 ĐÃ FIX: Thêm lại đầy đủ Trash2, PlusCircle để phần thêm ảnh/xóa ảnh hoạt động bình thường
import { 
  ...
  PenTool, Database, Calculator, Save, Search, Eye, Trash2, PlusCircle
} from "lucide-react";
...
const CreateAssignment = () => {
  ...
  const [newAssignment, setNewAssignment] = useState({ 
    title: "", targetClass: "", subject: "Toán", duration: "45", dueDate: getDefaultDueDate()
  });
      ...
      try {
        ...
        if (data.questions && data.questions.length > 0) {
          ...
          const formattedQuestions = data.questions.map((item) => {
            ...
            if (q.type === 'multiple_choice') mcqCount++;
            else essayCount++;
  ...
  const availableSets = [...new Set(questions
    .filter(q => q.grade === bankGrade && q.subject === bankSubject)
    .map(q => q.questionSet || "Ngân hàng chung")
    .filter(Boolean)
  )];
  ...
  const handleEssayCountChange = (e) => {
    ...
    if (count > newPts.length) {
        while (newPts.length < count) newPts.push("");
    } else {
        newPts.length = count;
    }
  ...
  const handleGenerateSlots = () => {
    ...
    for (let i = 0; i < mcqCount; i++) {
        ...
        generatedSlots.push({
            tempId, type: "multiple_choice", content: "", options: ["", "", "", ""],
            correctAnswer: "A", difficulty: "medium", imageFile: null, previewUrl: ""
        });
    ...
    for (let i = 0; i < essayCount; i++) {
        ...
        generatedSlots.push({
            tempId, type: "essay", content: "", options: [], correctAnswer: "",
            difficulty: "medium", imageFile: null, previewUrl: ""
        });
  ...
  const fillEmptySlots = (importedQs) => {
    ...
    const existingContents = new Set(
        newManuals.map(q => q.content.trim().toLowerCase()).filter(c => c !== "")
    );

    for (let i = 0; i < newManuals.length; i++) {
      if (importIdx >= importedQs.length) break;

      if (newManuals[i].content.trim() === "") {
        const impQ = importedQs[importIdx];
        ...
        if (impQ.options) {
            try {
                parsedOptions = typeof impQ.options === 'string' ? JSON.parse(impQ.options) : impQ.options;
        ...
        newManuals[i] = {
          ...
          options: newManuals[i].type === 'multiple_choice' ? parsedOptions : [],
    ...
    if (importIdx < importedQs.length && filledCount > 0) alertMsg += `\n⚠️ Khung đã đầy, còn thừa ${importedQs.length - importIdx} câu không được điền.`;
  ...
  // ==========================================
  // XỬ LÝ NHẬP THỦ CÔNG & UPLOAD ẢNH
  // ==========================================
  ...
  const handleManualImageChange = (tempId, e) => {
    ...
    if (file) {
      ...
      setManualQuestions(manualQuestions.map(q => q.tempId === tempId ? { ...q, imageFile: file, previewUrl: previewUrl } : q));
    }
  };

  const handleRemoveManualImage = (tempId) => {
    setManualQuestions(manualQuestions.map(q => q.tempId === tempId ? { ...q, imageFile: null, previewUrl: "" } : q));
  };

  const handleClearSlot = (tempId) => {
    if(!window.confirm("Bạn có chắc chắn muốn xóa nội dung và làm trống khung này?")) return;
    setManualQuestions(manualQuestions.map(q => {
        if (q.tempId === tempId) {
  ...
  const handleSubmit = async (actionType) => {
    ...
    try {
      ...
      if (!isValid) { 
          setLoading(false); 
          return alert("Vui lòng điền nội dung đầy đủ cho tất cả các khung trống!"); 
      }
      ...
      for (let q of manualQuestions) {
          ...
          if (contentSet.has(normalized)) {
              setLoading(false);
              return alert(`⚠️ Lỗi: Câu hỏi "${q.content.substring(0, 30)}..." bị trùng lặp trong đề. Vui lòng kiểm tra và sửa lại câu bị trùng!`);
          }
      ...
      const questionsToSave = manualQuestions.map(q => ({
          _id: q._id, tempId: q.tempId, content: q.content, type: q.type, options: q.options, 
      ...
      if (actionType === 'draft') alert("💾 Đã lưu nháp bài tập thành công!");
      else alert("✅ Giao bài thành công!");
      
      navigate("/teacher-dashboard");
    } catch (err) { 
        alert(err.response?.data?.message || "Lỗi xử lý! Vui lòng thử lại."); 
    } finally { 
        setLoading(false); 
    }
  };
  ...
  return (
    <div className="min-h-screen bg-sky-50/40 p-4 sm:p-6 md:p-10 font-sans text-slate-800 relative">
      <div className="max-w-5xl mx-auto">
        <Button variant="ghost" onClick={() => navigate("/teacher-dashboard")} className="text-sky-600 hover:text-sky-700 hover:bg-sky-100 font-bold px-3 py-2 sm:px-0 mb-4 sm:mb-6 h-auto w-max">
                  ...
                  <div>
                    ...
                    {!isPointsValid && (
                        <p className="text-sm font-medium mt-1">
                          {roundedTotal < 10 
                             ? `⚠️ Còn thiếu ${(10 - roundedTotal).toFixed(2)} điểm. Hãy điều chỉnh điểm Tự luận!` 
                             : `⚠️ Đang thừa ${(roundedTotal - 10).toFixed(2)} điểm. Hãy điều chỉnh điểm Tự luận!`}
                        </p>
                    )}
                  </div>
                </div>
          ...
          <CardContent className="p-4 sm:p-8">
            <form onSubmit={(e) => e.preventDefault()} className="space-y-8">
              ...
              <div className="space-y-4">
                <h3 className="text-lg sm:text-xl font-black text-sky-900 border-b border-sky-100 pb-2">1. Thông vị chung</h3>
                <Input placeholder="Nhập tên bài tập (VD: Bài kiểm tra 15 phút)..." className="h-12 sm:h-14 rounded-xl bg-slate-50 font-bold text-base sm:text-lg border-sky-100 focus-visible:ring-sky-500" value={newAssignment.title} onChange={(e) => setNewAssignment({...newAssignment, title: e.target.value})} required />
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <div className="space-y-1.5"><label className="text-xs sm:text-sm font-bold text-slate-500 ml-1">Môn học</label><Select value={newAssignment.subject} onValueChange={(val) => setNewAssignment({...newAssignment, subject: val})} required><SelectTrigger className="h-11 sm:h-12 rounded-xl bg-slate-50 font-bold border-sky-100 w-full"><SelectValue placeholder="Chọn môn" /></SelectTrigger><SelectContent><SelectItem value="Toán">Toán</SelectItem><SelectItem value="Ngữ Văn">Ngữ Văn</SelectItem><SelectItem value="Tiếng Anh">Tiếng Anh</SelectItem></SelectContent></Select></div>
                ...
                {step === 2 && (
                  <Card className="max-w-2xl mx-auto rounded-3xl border-sky-100 shadow-lg animate-in fade-in slide-in-from-bottom-4 duration-300">
                      ...
                      <CardContent className="p-6 space-y-6">
                          <div className="space-y-4">
                             {templateConfig.type !== 'full_essay' && (
                               <div className="space-y-2">
                                 ...
                                 <Input type="number" min="0" value={templateConfig.mcqCount} onChange={(e) => setTemplateConfig({...templateConfig, mcqCount: e.target.value})} className="h-12 rounded-xl border-slate-200 font-bold text-lg" />
                               </div>
                             )}
                             {templateConfig.type !== 'full_mcq' && (
                               <div className="space-y-2">
                                 ...
                                 <Input type="number" min="0" value={templateConfig.essayCount} onChange={handleEssayCountChange} className="h-12 rounded-xl border-slate-200 font-bold text-lg" />
                               </div>
                             )}
                                 ...
                                 <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                                   {templateConfig.essayPoints.map((pt, idx) => (
                                     <div key={idx} className="flex flex-col gap-1">
                                       ...
                                       <Input 
                                          type="number" step="0.25" min="0" placeholder="VD: 2.5"
                                          value={pt} 
                                          onChange={(e) => {
                                             const newPts = [...templateConfig.essayPoints];
                                             newPts[idx] = e.target.value;
                                             setTemplateConfig({...templateConfig, essayPoints: newPts});
                                          }} 
                                          className="h-10 rounded-lg border-indigo-200 font-bold bg-indigo-50 text-indigo-900 focus-visible:ring-indigo-500" 
                                       />
                                     </div>
                                   ))}
                                 </div>
                          ...
                          <div className={`p-4 rounded-xl border flex items-start gap-3 ${pointStatus.valid ? 'bg-emerald-50 border-emerald-200' : 'bg-rose-50 border-rose-200'}`}>
                ...
                {step === 3 && (
                  <div className="animate-in fade-in duration-500">
                    <div className="flex justify-between items-center mb-4">
                      ...
                      {!id && (
                          <Button type="button" onClick={() => {if(window.confirm("Các câu hỏi hiện tại sẽ bị xóa. Bạn có chắc chắn muốn làm lại?")) setStep(1)}} variant="ghost" className="text-rose-500 hover:bg-rose-50">
                              Làm lại cấu trúc đề
                          </Button>
                      )}
                    </div>
                    ...
                    <div className="mt-4 border border-sky-100 bg-sky-50/30 rounded-2xl p-3 sm:p-4 md:p-6">
                            ...
                            <Card key={q.tempId} className={`shadow-sm relative overflow-hidden transition-all ${isSlotEmpty ? 'border-sky-300 bg-white shadow-md' : 'border-sky-200'}`}>
                              ...
                              <CardHeader className="bg-slate-50/50 py-3 px-4 sm:px-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 border-b border-sky-50">
                                <div className="flex flex-wrap items-center gap-3 sm:gap-4 w-full sm:w-auto">
                                  <CardTitle className="text-sm sm:text-base font-black text-sky-900 whitespace-nowrap">
                                      Câu {index + 1} <span className="text-slate-400 font-medium text-xs sm:text-sm ml-1">({q.type === 'essay' ? 'Tự luận' : 'Trắc nghiệm'})</span>
                                  </CardTitle>
                                  ...
                                  <div className="flex items-center bg-white border border-sky-200 rounded-lg px-2 py-1 gap-2 shadow-sm ml-auto sm:ml-0">
                                    ...
                                    {q.type === "essay" ? (
                                      <input 
                                        type="number" step="0.1" className="w-12 sm:w-16 text-center font-black text-sky-600 focus:outline-none" 
                                        value={questionPoints[q.tempId] === 0 ? "" : questionPoints[q.tempId]} 
                                        placeholder="Nhập" onChange={(e) => handleEssayPointChange(q.tempId, e.target.value)}
                                      />
                                    ) : (
                                      <input 
                                        type="number" className="w-12 sm:w-16 text-center font-black text-slate-400 bg-transparent focus:outline-none cursor-not-allowed" 
                                        value={(questionPoints[q.tempId] || 0).toFixed(2)} readOnly title="Điểm trắc nghiệm được chia tự động"
                                      />
                                    )}
                                  </div>
                                </div>
                                <div className="flex items-center gap-2 shrink-0 self-end sm:self-auto">
                                  <Button type="button" onClick={() => handleClearSlot(q.tempId)} variant="ghost" size="icon" title="Xóa nội dung, làm trống khung" className="h-8 w-8 text-slate-400 hover:bg-rose-50 hover:text-rose-500"><Eraser className="w-4 h-4"/></Button>
                                </div>
                              </CardHeader>
                              ...
                              <CardContent className="p-4 sm:p-5 space-y-4">
                                <div className="flex flex-col md:flex-row gap-3 sm:gap-4">
                                  <Textarea 
                                    placeholder="Gõ trực tiếp nội dung câu hỏi hoặc rót từ file Word/Kho..." 
                                    className={`flex-1 rounded-xl min-h-[100px] sm:min-h-[120px] font-medium bg-white text-sm sm:text-base ${isSlotEmpty ? 'border-dashed border-2 border-slate-300' : 'border-sky-100'}`} 
                                    value={q.content} onChange={(e) => handleManualChange(q.tempId, 'content', e.target.value)} 
                                  />
                                  <div className="w-full md:w-36 shrink-0 h-[120px] sm:h-[120px]">
                                    {q.previewUrl ? (
                                      <div className="relative w-full h-full rounded-xl border border-sky-200 overflow-hidden shadow-sm group/img">
                                          <img src={q.previewUrl} alt="Preview" className="absolute inset-0 w-full h-full object-cover" />
                                          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover/img:opacity-100 transition-opacity flex items-center justify-center">
                                              <button type="button" onClick={() => handleRemoveManualImage(q.tempId)} className="bg-rose-500 text-white rounded-full p-2 hover:scale-110 transition-transform"><Trash2 className="w-4 h-4"/></button>
                                          </div>
                                      </div>
                                    ) : (
                                  ...
                                  <div className="bg-sky-50/50 p-3 sm:p-4 rounded-xl border border-sky-100 space-y-3 mt-4">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                      {['A', 'B', 'C', 'D'].map((optLabel, optIdx) => (
                                        <div key={optIdx} className="flex items-center gap-2">
                                          ...
                                          <Input placeholder={`Đáp án ${optLabel}`} className={`h-11 rounded-xl bg-white text-sm sm:text-base ${isSlotEmpty ? 'border-dashed border-slate-300' : 'border-sky-100'}`} value={q.options[optIdx]} onChange={(e) => handleManualOptionChange(q.tempId, optIdx, e.target.value)} />
                                        </div>
                                      ))}
                                    </div>
                                    <div className="flex flex-col sm:flex-row sm:items-center justify-between pt-3 border-t border-sky-100 gap-3">
                                      ...
                                      <Select value={q.correctAnswer} onValueChange={(val) => handleManualChange(q.tempId, 'correctAnswer', val)}>
                                        <SelectTrigger className="h-10 w-full sm:w-28 bg-white text-rose-600 font-bold border-rose-200 rounded-xl"><span className="truncate">{q.correctAnswer ? `Câu ${q.correctAnswer}` : "Chọn"}</span></SelectTrigger>
                      ...
                      {creationMethod === "upload" && (
                        <div className="bg-white p-4 sm:p-6 rounded-2xl border border-sky-100 shadow-sm text-center">
                          ...
                          <p className="text-slate-500 text-xs sm:text-sm mb-4">Tải file Word lên, hệ thống sẽ tự động bỏ qua các câu trùng lặp và rót dữ liệu vào khung trống.</p>
                        ...
                        <div className="border border-sky-200 rounded-xl sm:rounded-2xl overflow-hidden bg-white shadow-sm">
                          <div className="bg-sky-50 px-3 sm:px-4 py-4 flex flex-col space-y-4 border-b border-sky-100">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                              <Select value={bankSubject} onValueChange={setBankSubject}><SelectTrigger className="h-10 bg-white border-sky-200 font-medium"><SelectValue placeholder="Chọn môn" /></SelectTrigger><SelectContent><SelectItem value="all">Tất cả môn</SelectItem><SelectItem value="Toán">Toán</SelectItem><SelectItem value="Ngữ Văn">Ngữ Văn</SelectItem><SelectItem value="Tiếng Anh">Tiếng Anh</SelectItem></SelectContent></Select>
                              <Select value={bankGrade} onValueChange={setBankGrade}><SelectTrigger className="h-10 bg-white border-sky-200 font-medium"><SelectValue placeholder="Chọn khối" /></SelectTrigger><SelectContent><SelectItem value="all">Tất cả khối</SelectItem><SelectItem value="6">Khối 6</SelectItem><SelectItem value="7">Khối 7</SelectItem><SelectItem value="8">Khối 8</SelectItem><SelectItem value="9">Khối 9</SelectItem></SelectContent></Select>
                              <Select value={bankSetName} onValueChange={setBankSetName} disabled={!isSetSelectionEnabled}><SelectTrigger className={`h-10 bg-white border-sky-200 font-medium ${!isSetSelectionEnabled ? 'opacity-50' : ''}`}><SelectValue placeholder={isSetSelectionEnabled ? "Chọn Bộ đề" : "Chọn Khối & Môn trước"} /></SelectTrigger><SelectContent><SelectItem value="all">Tất cả Bộ đề</SelectItem>{availableSets.map((setName, idx) => (<SelectItem key={idx} value={setName}>{setName}</SelectItem>))}</SelectContent></Select>
                              <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" /><Input placeholder="Tìm nội dung..." className="pl-9 h-10 bg-white border-sky-200" value={bankSearch} onChange={e => setBankSearch(e.target.value)} /></div>
                            </div>
                          </div>
                ...
                <div className="pt-6 sm:pt-8 border-t border-slate-200">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <Button type="button" onClick={() => handleSubmit('published')} disabled={loading || !isPointsValid} className="w-full h-14 sm:h-16 rounded-xl sm:rounded-2xl bg-sky-500 hover:bg-sky-600 text-white font-black text-lg shadow-xl shadow-sky-200 transition-all active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed">
                        {loading ? <Loader2 className="animate-spin mr-2 h-5 w-5" /> : <CheckCircle className="mr-2 h-5 w-5" />} Phát hành Bài tập
                    </Button>
                  </div>
                </div>
              )}
