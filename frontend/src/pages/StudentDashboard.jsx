...
const exportFormalExcel = async (dataList, reportTitle, fileName, studentName) => {
  ...
  sheet.columns = [ { width: 10 }, { width: 45 }, { width: 20 }, { width: 25 }, { width: 15 } ];
...
const StudentDashboard = () => {
  ...
  const filteredHistory = completedAssignments.filter(sub => {
    ...
    if (historyDateFrom) matchDate = matchDate && (subDate >= new Date(historyDateFrom).setHours(0,0,0,0));
    if (historyDateTo) matchDate = matchDate && (subDate <= new Date(historyDateTo).setHours(23,59,59,999));
