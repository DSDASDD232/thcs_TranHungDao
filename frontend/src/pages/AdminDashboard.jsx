...
const AdminDashboard = () => {
  ...
  // const currentDay = new Date().getDate().toString(); // Đã thay đổi để mặc định là 'all' cho chế độ xem rộng hơn
                  ...
                  <Select value={selectedLbClassId} onValueChange={setSelectedLbClassId}>
                    <SelectTrigger className="w-full sm:w-[220px] bg-white border-slate-200 rounded-xl">
                      <span className="truncate">
                        {selectedLbClassId
                          ? `Lớp ${adminLeaderboard.find((c) => c._id === selectedLbClassId)?.className || ""}`
                          : "Chọn lớp"}
                      </span>
                    </SelectTrigger>
                    <SelectContent>
          ...
          {createMethod === "manual" ? (
            <form onSubmit={handleCreateUser} className="space-y-4 mt-6">
              ...
              <Select value={newUser.status} onValueChange={(val) => setNewUser({...newUser, status: val})}>
                <SelectTrigger className="h-11 rounded-xl font-medium border-sky-100 bg-white"><span className="truncate">{newUser.status === "active" ? "Hoạt động" : "Không hoạt động"}</span></SelectTrigger>
              ...
              {newUser.role === "teacher" && (
                <div className="p-4 bg-emerald-50/50 rounded-xl border-emerald-100 space-y-4">
                  <Select value={newUser.department} onValueChange={(val) => setNewUser({...newUser, department: val, subjects: []})}>
                    <SelectTrigger className="h-11 rounded-xl border-emerald-100 bg-white"><span className="truncate">{newUser.department ? `Tổ ${newUser.department}` : "Chọn Tổ chuyên môn"}</span></SelectTrigger>
                    <SelectContent>
                  ...
                  <Select value={newUser.qualification || ""} onValueChange={(val) => setNewUser({...newUser, qualification: val})}>
                    <SelectTrigger className="h-11 rounded-xl border-emerald-100 bg-white"><span className="truncate">{newUser.qualification || "Chọn trình độ"}</span></SelectTrigger>
                    <SelectContent>
          ...
          {editUser && (
            <form onSubmit={handleUpdateUser} className="space-y-4 pt-4">
              ...
              <Select value={editUser.status || "active"} onValueChange={(val) => setEditUser({...editUser, status: val})}>
                <SelectTrigger className="h-11 rounded-xl bg-white"><span className="truncate">{editUser.status === "active" ? "Hoạt động" : "Không hoạt động"}</span></SelectTrigger>
                <SelectContent><SelectItem value="active">Hoạt động</SelectItem><SelectItem value="inactive">Không hoạt động</SelectItem></SelectContent>
              </Select>
              {editUser.role === "teacher" && (
                <div className="p-4 bg-emerald-50/50 rounded-xl border-emerald-100 space-y-4">
                  <Select value={editUser.department || ""} onValueChange={(val) => setEditUser({...editUser, department: val, subjects: []})}>
                    <SelectTrigger className="h-11 rounded-xl bg-white"><span className="truncate">{editUser.department ? `Tổ ${editUser.department}` : "Chọn Tổ chuyên môn"}</span></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="">Không chọn</SelectItem>
                      <SelectItem value="KHTN">Tổ KHTN</SelectItem>
                      <SelectItem value="KHXH">Tổ KHXH</SelectItem>
                  ...
                  <Select value={editUser.qualification || ""} onValueChange={(val) => setEditUser({...editUser, qualification: val})}>
                    <SelectTrigger className="h-11 rounded-xl bg-white"><span className="truncate">{editUser.qualification || "Chọn trình độ"}</span></SelectTrigger>
                    <SelectContent>
                      <SelectItem value="">Chưa cập nhật</SelectItem>
                      <SelectItem value="Đại học">Đại học</SelectItem>
                      <SelectItem value="Thạc sĩ">Thạc sĩ</SelectItem>
